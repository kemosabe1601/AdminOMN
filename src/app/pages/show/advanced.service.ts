...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.name.toLowerCase().includes(term)
        || tables.listCount.toLowerCase().includes(term)
        || tables.days.toLowerCase().includes(term)
        || tables.endTime.toLowerCase().includes(term)
        // || pipe.transform(tables.age).includes(term)
        || tables.belong.toLowerCase().includes(term);
}
